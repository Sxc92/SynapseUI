...
export function useResourceTable() {
  ...
  // 先创建 Drawer Form 实例
  const drawerForm = createDrawerForm<ResourceData>({
    title: {
      create: $t('resource.create'),
      edit: $t('resource.edit'),
    ...
    api: {
      create: (data: any) => {
        // 验证：menuId 必选
        if (!data.menuId) {
          message.error($t('resource.selectMenuPlaceholder'));
          return Promise.reject(new Error($t('resource.selectMenuPlaceholder')));
        }
        // 确保不包含 id，明确是新增
        const { id, ...createData } = data as any;
        return addOrModifyResource(createData);
      },
  ...
  const result = createGrid<ResourceData>({
    tableTitle: $t('menus.resource'),
    id: 'resource-grid',
    pageSize: 10,
    formOptions,
    api: {
      getPage: getResourcePage,
    },
  ...
  // 获取选中记录
  function getSelectedRows(): ResourceData[] {
