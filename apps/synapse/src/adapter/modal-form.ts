...
import { computed, defineComponent, h, nextTick, reactive, ref, unref, watch } from 'vue';
 ...
 * API 方法接口
 */
export interface ModalFormApiMethods {
  /** 创建数据 */
...
export interface ModalFormOptions<T = any> {
  ...
  /** 表单配置 */
  formSchema:
    | ((mode: ModalFormMode, row?: T) => VbenFormSchema[])
    | VbenFormSchema[];
  /** API 方法 */
  api: ModalFormApiMethods;
  /** 弹窗宽度（默认 1200px，适合 Transfer 组件） */
  width?: number | string;
  /** 是否居中显示 */
  centered?: boolean;
  /** 是否全屏 */
  ...
  /** 是否可拖拽 */
  draggable?: boolean;
  /** 是否挂载到主内容区域（用于在内容区域居中或拖拽） */
  appendToMain?: boolean;
  /** 提交成功后的回调 */
  onSuccess?: (mode: ModalFormMode, data: Recordable<any>) => void;
  /** 提交失败后的回调 */
 ...
 * 创建 Modal Form 工厂函数
 * @param options 配置选项
 * @returns Modal Form 实例
 */
export function createModalForm<T = any>(
  ...
  // 当前模式
  const mode = ref<ModalFormMode>('create');
  // 当前行数据
  ...
  // 计算弹窗标题（确保始终有值，满足可访问性要求）
  const modalTitle = computed(() => {
    const titles = options.title || {};
    switch (mode.value) {
      case 'create': {
        return titles.create || $t('common.create') || 'Create';
      }
      case 'edit': {
        return titles.edit || $t('common.edit') || 'Edit';
      }
      case 'view': {
        return titles.view || $t('common.view') || 'View';
      }
      default: {
        return 'Modal'; // 确保始终有值
      }
  ...
  const [Form, formApi] = useVbenForm(
    reactive({
      commonConfig: {
        componentProps: {
          class: 'w-full modal-form-input', // 添加自定义类名，用于统一样式
        },
        // 使用 controlClass 来设置控件样式
        controlClass: 'modal-form-control', // 统一控件样式类
        // 垂直布局时不需要设置 labelWidth
        // labelWidth: 120, // 垂直布局时不需要固定标签宽度
        // 标签后显示冒号
        colon: true,
        // 表单项包装器样式：增加间距，更舒适
        wrapperClass: 'mb-4', // 表单项之间的间距
        // 标签样式：简洁的标签样式
        labelClass: 'text-sm font-medium text-gray-700', // 标签字体大小和粗细
      },
      // 使用垂直布局：标签和输入框分两行显示
      layout: 'vertical',
      // 使用栅格布局，支持字段并排显示（默认2列布局，可通过 wrapperClass 自定义）
      wrapperClass: options.wrapperClass || 'grid-cols-1 gap-6', // 单列布局，增加间距
      schema: formSchema,
  ...
  // 创建弹窗实例
  // 注意：title 和 description 必须始终有值（即使是空字符串），以满足可访问性要求
  // Modal 组件会自动使用 VisuallyHidden 包裹空的 title 或 description
  const [VbenModal, modalApi] = useVbenModal({
    title: unref(modalTitle), // 确保标题始终有值
    description: '', // 空描述，Modal 组件会使用 VisuallyHidden 包裹以满足可访问性要求
    loading: unref(loading),
    isOpen: unref(open),
    centered: options.centered ?? true, // 默认居中
    fullscreen: options.fullscreen ?? false,
    fullscreenButton: options.fullscreenButton ?? true,
    draggable: options.draggable ?? true, // 默认可拖拽
    appendToMain: options.appendToMain ?? false, // 是否挂载到主内容区域
  ...
  // 设置弹窗宽度和位置
  // 使用 watch 确保在 Modal 打开后正确设置样式
  watch(
    () => [open.value, options.width, options.centered, options.appendToMain],
    ([isOpen, width, centered, appendToMain]) => {
      if (isOpen && width) {
        nextTick(() => {
          ...
          const isAppendToMain = appendToMain ?? false;
          
          // 构建 class 字符串，使用 !important 覆盖默认的 w-[520px]
          // 注意：Tailwind 的 ! 前缀会生成 !important
          let modalClass = `!max-w-[${widthValue}] !w-[${widthValue}]`;
          
          // 如果 appendToMain 且不居中，在内容区域居中
          if (isAppendToMain && !isCentered) {
            modalClass += ' !mx-auto';
          }
  ...
  // 监听模式变化，更新弹窗状态
  // 注意：title 必须始终有值（非空字符串），description 可以为空字符串
  // Modal 组件会根据这些值自动显示 DialogTitle 和 VisuallyHidden 的 DialogDescription
  watch(
    [mode, loading, open, modalTitle],
    () => {
      const currentTitle = unref(modalTitle);
      modalApi.setState({
        title: currentTitle || 'Modal', // 确保标题始终有值（非空字符串），这样 v-if="title" 会显示 DialogTitle
        description: '', // 空字符串，Modal 组件会使用 VisuallyHidden 包裹 DialogDescription 以满足可访问性要求
        loading: unref(loading),
        isOpen: unref(open),
        footer: mode.value !== 'view',
        confirmText:
  ...
  return {
    ...
    Modal: defineComponent({
      name: 'ModalForm',
      setup() {
        // 在 setup 中直接访问闭包变量，添加安全检查避免 HMR 时出错
        return () => {
          // 安全检查：如果组件为 null（HMR 时可能发生），返回 null
          if (!VbenModal || !ModalContent) {
            console.warn('Modal components not ready, skipping render');
            return null;
          }
          return h(
            VbenModal,
